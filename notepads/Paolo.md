### Integrazione operativa tra U-Net e Noise2Void nel denoising di immagini
...
La scelta di utilizzare questa combinazione è particolarmente motivata dal nostro dataset, **Jump**, costituito da immagini di nuclei cellulari acquisite con microscopia a fluorescenza. Queste immagini presentano **rumore reale**, generato da fenomeni fisici come fluttuazioni di fotoni, rumore elettronico e imperfezioni ottiche, che rende impossibile ottenere una ground truth pulita. Applicando U‑Net insieme a Noise2Void, possiamo **addestrare la rete direttamente sulle immagini rumorose** e ottenere un output denoised che preserva fedelmente i dettagli biologici, migliorando la visibilità di strutture cellulari e facilitando analisi quantitative successive.

#### **1. Origine dei dati e natura del problema**

Il punto di partenza di questo lavoro è costituito dalle immagini del dataset Jump Cell Painting, una raccolta di immagini di cellule umane ottenute mediante microscopia fluorescente ad alta risoluzione, progettata per il profiling cellulare e l’analisi morfologica quantitativa. Il dataset include cellule trattate con differenti perturbazioni chimiche o genetiche, marcate con più fluorofori per evidenziare specifici componenti cellulari, come nuclei, mitocondri, reticolo endoplasmatico, apparato di Golgi e citoplasma. Ogni immagine è acquisita attraverso diversi canali fluorescenti, fornendo una rappresentazione multicanale dettagliata della morfologia e dello stato funzionale delle cellule.

Le immagini sono caratterizzate dalla presenza di rumore reale, derivante da fattori sperimentali e strumentali. Tra le principali sorgenti di rumore si annoverano il rumore di conteggio dei fotoni (Poisson noise), particolarmente rilevante in condizioni di bassa luminosità, e il rumore elettronico del sensore, generato dai circuiti di acquisizione e amplificazione. Anche piccole variazioni di illuminazione tra campioni o tra diverse sessioni di acquisizione possono introdurre disomogeneità spaziali nell’intensità del segnale. Inoltre, fenomeni come la sfocatura parziale o la cattura di piani focali multipli contribuiscono ulteriormente al degrado dell’immagine.

A differenza del rumore artificiale simulato, il rumore reale nel Jump Cell Painting dataset è complesso e non stazionario, variando in funzione del canale, della posizione all’interno della cella e delle condizioni sperimentali. Questa complessità limita l’uso dei metodi supervisionati tradizionali, che richiedono immagini “pulite” di riferimento. Infatti, l’acquisizione di immagini completamente prive di rumore risulta spesso impossibile, poiché aumentare l’esposizione o la luminosità per ridurre il rumore potrebbe danneggiare le cellule o alterare la loro fisiologia.

Di conseguenza, approcci self-supervised, come il Noise2Void (N2V), diventano fondamentali per addestrare modelli di denoising senza la necessità di immagini ground truth, sfruttando direttamente le immagini rumorose disponibili nel dataset Jump Cell Painting.
...
Le immagini del dataset Jump Cell Painting sono acquisite in formato a 16 bit e contengono più canali fluorescenti, ciascuno corrispondente a specifici componenti cellulari (nuclei, mitocondri, reticolo endoplasmatico, citoplasma, ecc.). I valori dei pixel possono avere scale molto diverse tra canali e tra immagini differenti, rendendo instabile il processo di addestramento se utilizzati direttamente.

Per questo motivo, il primo passo del pre-processing consiste nella normalizzazione delle intensità di ciascun canale, riportando i valori dei pixel in un intervallo standard, generalmente compreso tra 0 e 1. Questa normalizzazione riduce la variabilità artificiale tra immagini, migliora la stabilità numerica durante il training e facilita la convergenza dell’algoritmo di apprendimento.

Successivamente, le immagini vengono suddivise in patch multicanale di dimensione fissa (ad esempio 64×64 pixel). Questa operazione presenta diversi vantaggi:

- aumenta il numero di campioni disponibili per l’addestramento,

- riduce il carico computazionale,

- consente alla rete di concentrarsi su regioni locali dell’immagine, dove rumore e strutture cellulari si manifestano in modo più omogeneo.

Nel contesto di Noise2Void, il pre-processing assume un ruolo ancora più centrale. Durante il training, viene applicato un mascheramento casuale dei pixel all’interno delle patch: alcuni pixel vengono intenzionalmente nascosti o sostituiti con valori derivati dai loro vicini, costringendo la rete a predirne il valore basandosi esclusivamente sul contesto spaziale circostante. Questo meccanismo permette di addestrare il modello senza immagini pulite di riferimento, sfruttando le proprietà statistiche del rumore reale presente nelle immagini multicanale.
Il pre-processing non rappresenta un semplice passaggio preliminare, ma una fase fondamentale che condiziona direttamente l’efficacia dell’addestramento e la qualità finale del denoising ottenuto dalla combinazione U-Net + Noise2Void.

##### **2.1 Lettura e rappresentazione numerica**

Le immagini del dataset Jump Cell Painting, in formato TIF a 16 bit, vengono inizialmente caricate in memoria come matrici tridimensionali di valori numerici, in cui ciascun elemento della matrice rappresenta l’intensità luminosa di un pixel in un determinato canale fluorescente. In questa rappresentazione:

- le prime due dimensioni identificano la posizione spaziale del pixel (righe e colonne),

- la terza dimensione corrisponde ai canali biologici acquisiti (ad esempio nuclei, mitocondri, citoplasma, ecc.),

- il valore numerico indica il livello di fluorescenza misurato dal sensore per quel pixel e quel canale.

L’utilizzo del formato a 16 bit permette di preservare una dinamica di intensità elevata, fondamentale per catturare variazioni sottili del segnale biologico che andrebbero perse in una codifica a 8 bit. Questo è particolarmente importante nelle immagini multicanale di microscopia fluorescente, dove il contrasto tra segnale e rumore può essere molto ridotto e le informazioni significative si trovano spesso in variazioni di intensità minute.

La rappresentazione numerica multicanale consente alla rete neurale convoluzionale di elaborare direttamente le immagini come array di numeri reali, applicando operazioni matematiche come convoluzioni, somme pesate e funzioni di attivazione. In questo modo, l’intero processo di denoising può essere formalizzato come un problema di apprendimento su matrici di valori reali, senza introdurre approssimazioni o perdite di informazione dovute a conversioni premature o compressioni del dato.

La lettura delle immagini come matrici tridimensionali costituisce il passaggio fondamentale che collega i dati sperimentali reali al modello matematico e computazionale rappresentato dalla rete U-Net, fornendo una base informativa completa e adeguata per l’addestramento e il denoising multicanale.
...
6. **Krull, A., Buchholz, T.O., & Jug, F. (2019). *Noise2Void – Learning Denoising from Single Noisy Images*. CVPR 2019.** https://arxiv.org/abs/1811.10980
